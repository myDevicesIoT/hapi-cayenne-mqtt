...
var CayenneMQTT = function (options, logger) {
    this.config = {
        ...
        mqttConfig: {
            ...
            clientId: options.MQTT_CLIENT_ID || "bridge-" + hostname + "-" + process.pid
        },
    }
...
CayenneMQTT.prototype.rawWrite = function (username, thingId, channel, value, type, unit) {
    ...
    var payload = util.format("%s,%s=%s", type, unit, value);
...
CayenneMQTT.prototype.jsonWrite = function (username, thingId, json) {
    // TODO use /data/json topic
    // this.publish(getTopic(username, thingId, MQTT_DATA, 'json'), JSON.stringify(json));
    for (s in json) {
        var sensor = json[s];
        this.rawWrite(username, thingId, sensor.channel, sensor.value, sensor.type, sensor.unit);
    }
}
